...
// get Scancode func
int TestScancode3() {
  unsigned int MaskCode[KeyOutputPinStop - KeyOutputPinStart +1];
  unsigned int Code[KeyOutputPinStop - KeyOutputPinStart +1];
  ...
  MyKeyb->ScanKeyMatrixLow(MaskCode);
   ...
   while (Encodr.isPress()==0) {
    ...
    do {
      ...
      unsigned char X = 1;
      unsigned char Y = 40;
      for (unsigned char OutCounter = 0; OutCounter < KeyOutputPinStop - KeyOutputPinStart + 1; OutCounter++) {
        Y = 40; 
        unsigned int CodeByte = MaskCode[OutCounter];
        for (unsigned char ByteNum = 0; ByteNum < KeyInputPinStop - KeyInputPinStart + 1; ByteNum++) {
          if(CodeByte & 0x01){
            u8g.drawBox(Y,X,2,2);
          }else{
            u8g.drawPixel(Y,X);
      ...
      u8g.print(MyKeyb->ScanCodeToKeyCode(ScanCode, KeyCodes), DEC);
      Encodr.tick();  
    } while ( u8g.nextPage() );
...
int TestScancode2() {
  ...
  while (1) {
    ...
    if (Encodr.isLeft()) {
      OutNum++;
      if (OutNum > KeyOutputPinStop - KeyOutputPinStart +1) {
        OutNum = 0;
      }
    }
    ...
    if (Encodr.isRight()) {
      OutNum--;
      if (OutNum < 0) {
        OutNum = KeyOutputPinStop - KeyOutputPinStart +1;;
      }
    }

    Pin = OutNum + KeyOutputPinStart;
    ...
    for (int CodeCounter = 0; CodeCounter < KeyInputPinStop - KeyInputPinStart +1; CodeCounter++) {
      if (Code & 0x01 == 1) {
        ...
        int Key = MyKeyb->ScanCodeToKeyCode(Scan, KeyCodes);
        if (Key != 0) {
